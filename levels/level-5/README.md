...
Uses gRPC for get/set/list. Reads use Deployment and intent caches. Set persists a per-Deployment ReplicaIntent; the elected controller reconciles it and writes status.
...
The wrapper forwards additional make arguments and fixes LEVEL=5.
...
helm template replica-control "$repo_root/charts/replica-control" \
  ...
  --values "$repo_root/levels/level-5/values.yaml"
```
